...
class OverviewPage(BaseRenderer):
    ...
    def render(self, data) -> str:
        ...
        # 同比起止月（从年度累计配置读）
        cur_start_m, cur_end_m, cur_year = 1, 6, pd.Timestamp.now().year
        ...
        yoy_period = f"{cur_year}年{cur_start_m:02d}-{cur_end_m:02d}月"

        hero = hero_rings_html(
            t_inc, t_inc_tgt, t_pay, t_pay_tgt,
            "", "收入", "回款",
            inc_yoy=yoy_inc, pay_yoy=yoy_pay,
            yoy_period=yoy_period, total_tgt=None, total_label="",
        ...
        return self.wrap_page(
            range_banner_html(annual_range)
            + hero
